...
#                 query_string = "abcdnet_score > 0.97 and abs_deta_jj > 6 and M_jj > 1000"
...
vbsvvh.df["cond_abcdnet"] = vbsvvh.df["abcdnet_score"] > 0.97
vbsvvh.df["cond_vbs"] = vbsvvh.df["abs_deta_jj"] > 6.0
vbsvvh.df["cond_M_jj"] = vbsvvh.df["M_jj"] > 1000

print("Entries with abcdnet_score > 0.97:", vbsvvh.df["cond_abcdnet"].sum())
print("Entries with abs_deta_jj > 6.0:", vbsvvh.df["cond_vbs"].sum())
print("Entries with M_jj > 1000:", vbsvvh.df["cond_M_jj"].sum())
...
vbsvvh.df["vbs_cut"] = vbsvvh.df.eval("abs_deta_jj > 6.0")
vbsvvh.df["regionA"] = vbsvvh.df.eval(
    "abcdnet_score > 0.97 and abs_deta_jj > 6 and M_jj > 1000"
)
vbsvvh.df["regionB"] = vbsvvh.df.eval(
    "abcdnet_score > 0.97 and (abs_deta_jj < 6 or M_jj < 1000)"
)
...
print("Entries passing abcdnet_cut (abcdnet_score > 0.97):", vbsvvh.df["abcdnet_cut"].sum())
print("Entries passing vbs_cut (abs_deta_jj > 6.0):", vbsvvh.df["vbs_cut"].sum())
print("Entries in region A (abcdnet_score > 0.97 and abs_deta_jj > 6 and M_jj > 1000):", vbsvvh.df["regionA"].sum())
print("Entries in region B (abcdnet_score > 0.97 and (abs_deta_jj < 6 or M_jj < 1000)):", vbsvvh.df["regionB"].sum())
print("Entries in region C (abcdnet_score < 0.97 and abs_deta_jj > 6 and M_jj > 1000):", vbsvvh.df["regionC"].sum())
print("Entries in region D (abcdnet_score < 0.97 and (abs_deta_jj < 6 or M_jj < 1000)):", vbsvvh.df["regionD"].sum())
...
print("Entries in region D (post filter):", vbsvvh.df[vbsvvh.df["regionD"]].shape[0])
    ...
    with uproot.open(f"{BASEDIR}/vbsvvhjets_semimerged/output_{TAG}/Run2/VBSVVH.root") as f:
...
vbsvvh.df.loc[vbsvvh.df.is_signal, "event_weight"] = vbsvvh.df[vbsvvh.df.is_signal].orig_event_weight.values
...
#
# with uproot.open(f"{BASEDIR}/signal_hadded_abcdnet.root") as f:
...
datacard_name = "my_basic_datacard2.dat"  # Example name, adapt as needed
