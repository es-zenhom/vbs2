...
vbs.df["presel"] = vbs.df.eval(
    "objsel and hbbfatjet_xbb > 0.8 and ld_vqqfatjet_xwqq > 0.6 and HT_fat > 1100 and tr_vqqjet_eta > -2.5 and tr_vqqjet_eta < 2.5 and ld_vqqjet_eta > -2.5 and ld_vqqjet_eta < 2.5"
)
...
for name in vbs.df.name.unique():
    ...
    presel_cut = Cut(
        "SemiMerged_PreselectionDiverted",
        n_pass=n_pass_raw,
    ...
    n_pass_wgt = df.event_weight.sum()
    global_cut = Cut(
        "SemiMerged_SignalRegionGlobalCuts",
        n_pass=n_pass_raw,
        n_pass_weighted=n_pass_wgt,
    ...
    ynotx_cut = Cut(
        "SemiMerged_SignalRegionYNotXCut",
        n_pass=n_pass_raw,
        n_pass_weighted=n_pass_wgt,
